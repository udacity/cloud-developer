...
export class TodoAccess {
  ...
  // QUESTION: WITH THE SCHEMA 2 of dynamnoDB, how can I accomplish this with todoId as index instead of a partion/sort key?
  ...
  // async addTodoAttachmentUrl(todoId: string, attachmentUrl: string, newItem: TodoItem) {
  async addTodoAttachmentUrl(newItem: TodoItem) {
    await this.docClient.put({
      TableName: this.todosTable,
      Item: newItem
    }).promise();
