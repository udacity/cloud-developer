...
describe('concat', () => { 
  it('should concat "abraka" and "dabra" to "abrakadabra"', () => {
    const result = concat('abraka', 'dabra');
    expect(result).to.equal('abrakadabra');
  });

  it('should throw an error if one argument is undefined', () => {
    expect(() => concat(null, null)).to.throw('Arguments must not be empty!');
  });
