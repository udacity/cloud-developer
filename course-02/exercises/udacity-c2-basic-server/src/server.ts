...
(async () => {
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    ...
    let matching_car = cars.filter((car) => car.id == id);

    if (matching_car && matching_car.length == 0) {
      return res.status(400).send(`no matching cars found`);
    } else {
      return res.status(200).send(matching_car);
    }
  });
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    ...
    return res.status(201).send({ make: make, type: type, model: model, cost: cost, id: id });
  });
