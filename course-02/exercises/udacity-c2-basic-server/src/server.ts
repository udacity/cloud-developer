...
>>>>>>> b8bcdd162ae86303de0e6cca0732fba3c468cadd
...
>>>>>>> b8bcdd162ae86303de0e6cca0732fba3c468cadd
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    const { id } = req.params;
    if (!id) res.status(400).send("id is requeried");
    let cars_list = cars;
    cars_list = cars.filter((car) => car.id === Number(id));
    if (!cars_list.length) return res.status(404).send("id not found");
    return res.status(200).send(cars_list);
  });
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    const { id, type, cost, model,make } = req.body;
    if (!id||!type||!cost||!model||!make) res.status(404).send("data requeried");
    cars.push({ make: make, type: type, model: model, cost: cost, id: id })
